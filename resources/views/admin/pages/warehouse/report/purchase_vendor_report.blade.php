    ...
    <div class="content-wrapper">
        @if(request()->get('vendor_name'))
            ...
            <div class="row">
                ...
                <div class="col-md-9">
                    @php
                        $PurchaseData = $Model('PurchaseVendor')::where('vendor_name', request()->get('vendor_name'));
                        if(request()->get('from_date')){
                             $PurchaseData =  $PurchaseData->whereBetween('created_at', [request()->get('from_date'), request()->get('to_date')]);
                        }
                    ...
                    <table id="PpiDataTable" class="table table-sm" style="width:100%">
                        ...
                        @foreach($PurchaseData as $item)
                            <tr>
                                <td>{{$item->id}}</td>
                                <td>{{$Model('Product')::where('id', $item->product_id)->first()->name}}</td>
                                <td> {{$item->qty}} </td>
                                <td> {{$item->price ?? 0}} </td>
                   ...
                   buttons: [
                       {
                           extend: 'excel',
                           text: 'Excel Export',
                           title: `Purchase Report Of Vendor {{request()->get('vendor_name')}} ({{date('d M Y')}})`,
                           split: [ 'pdf', 'csv'],
                       }
                   ],
