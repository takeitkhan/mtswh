...
@section('title')
    Faulty Product Report of {{$product->name}}
@endsection

@section('onlytitle')
    Faulty Product of <span class="text-primary">{{$product->name}}</span>
@endsection
                ...
                <div class="card card-tale">
                    <div class="card-body">
                        ...
                        <p class="font-30 mb-2">
                            @php
                                $totals = $Model('FaultyProduct')::where('id', $product->id)->first();
                            @endphp
                            <span id="total">{{$totals->faulty_product + $totals->faulty_product_bundle}} </span>
                            <span class="font-19">
            ...
            <div class="col-md-9">
                ...
                @php
                    $ppis = $Model('PpiProduct')::leftjoin('ppi_bundle_products', 'ppi_bundle_products.ppi_product_id', 'ppi_products.id')
                            ...
                            ->where('ppi_products.product_id', $product->id)->where('ppi_products.health_status', 'Faulty')->get()->groupBy('ppi_id')->toArray();
                    //dd($ppis);
                @endphp
                ...
                buttons: [
                    {
                        extend: 'excel',
                        text: 'Excel Export',
                        title: 'Faulty Product of 7/8 Feeder Cable Copper ({{date('d M Y')}})',
                        split: [ 'pdf', 'csv'],
                    }
                ],
