 ...
 */
$('select#spi_type').on('change', function(){
...
$("#spi_source").empty().append('<div class="row ppiAppen ppiAppend0">'+sourceTemplate+'</div>');
// let nb = '<div class="row ppiAppen ppiAppend0">'+sourceTemplate.html()+'</div>';
// $(sourceTemplate).appendTo("#spi_source");
...
$('#spi_source .ppiAppend0').on('change', 'select#main_source', function(){
...
$("#spi_source").on('change', 'select#source_child', function(){
    let id = ++i;
    ...
    if(sourceChild == 'Haschild'){
        ...
        let warehouseSiteInput = `<div class="col-md-10 form-group d-block">
                                <input class="form-control warehouse_name" 
                                    id="spi_id_picker" 
                                    type="text" 
                                    value="" name="main_source[${id}][source]" 
                                    placeholder = "${sourceChild} name" 
                                    data-source_type="${sourceChild}"
                                    required="required"/>
...
$('#source_warehouse_site').on('blur', '#spi_id_picker', function () {
    let who_source = $(this).val();
    let source_type = $(this).data('source_type'); // cleaner than placeholder split
    let warehouse_code = $('input[name="warehouse_code"]').val(); // grab hidden field value

    if (who_source !== '') {
        $.ajax({
            url: `/${warehouse_code}/spi/spi-id-lookup`,
            method: 'GET',
            data: {
            ...
            success: function (response) {
                $('#spi_lookup_result').remove();
                $('#source_warehouse_site').append(`<div id="spi_lookup_result">${response}</div>`);
            },
            error: function () {
                alert('SPI lookup failed');
            }
        });
    }
});
 ...
 */
$(function(){
    $('#spi_source').on('change','select', function(){
        sourceBreadcrumb();
    });
});
